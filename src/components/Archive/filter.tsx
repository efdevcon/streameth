        ...
        <section className={className}>
            ...
            {open && <div className={css.panel}>
                {Object.keys(props.filters).map(key => {
                    return <div key={key} className={css.filter}>
                        <div className={css.header} onClick={() => togglePanels(key)}>
                            <p>{key}</p>
                            <span className={css.icon}>
                                {filters.includes(key) ? <IconExpandMore /> : <IconExpandLess />}
                            </span>
                        </div>
                        {filters.includes(key) && <div className={css.tags}>
                            {props.filters[key].map(i => {
                                const active = activeFilter[key] && activeFilter[key].includes(i)
                                return <span
                                    key={i}
                                    className={active ? css.active : css.tag}
                                    onClick={() => filter(key, i)}>
                                    {i}</span>
                            })}
                        </div>}
                    </div>
